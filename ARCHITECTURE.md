...
| 测试 | pytest (756 collected，通过数随环境) + behave (211 scenarios / 1369 steps) |
...
tests/unit/              # pytest 单元测试（756 个 collected，通过数随环境）
...
```bash
# pytest
756 collected（通过数随环境：开发机 756 passed / CI 708+15 skip / 无 MySQL 沙箱 714+9 err）
...
| — | 测试覆盖补全 | P0 parallel | ✅ 已修复（756 测试 + 真实断言 + MySQL 并发脚本） |

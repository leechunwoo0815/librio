...
| 测试 | pytest (925 collected，通过数随环境) + behave (211 scenarios / 1369 steps) |
...
tests/unit/              # pytest 单元测试（925 个 collected，通过数随环境）
...
```bash
# pytest
925 collected（通过数随环境：开发机 925 passed / CI 708+15 skip / 无 MySQL 沙箱 714+9 err）
...
| — | 测试覆盖补全 | P0 parallel | ✅ 已修复（789 测试 + 真实断言 + MySQL 并发脚本） |

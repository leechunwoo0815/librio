...
def step_reading_stats(context):
    ...
    book = Book(isbn="978REP1", title="ReportBook", author="A", ar_value=2.0,
                age_min=5, age_max=9, word_count=3000)
    context.db.add(book); context.db.commit()
...
def step_quiz_passed(context):
    ...
    book = Book(isbn="978REP2", title="QuizReportBook", author="B", ar_value=2.0,
    ...
    for i in range(3):
        quiz = Quiz(
            child_id=context.child.id, book_id=book.id,
            status=Quiz.STATUS_COMPLETED, score=90, total_questions=5,
...
def step_teacher_comment_exists(context):
    ...
    svc.add_teacher_comment(report["id"], teacher_id=1, comment="表现优秀，建议继续学习")

...
@given(u'孩子已读完{count:d}本书共{words:d}词')
...
def step_current_level_a(context):
    ...
    if not level:
        ...
        context.db.add(level); context.db.commit()
    cl = ChildLevel(child_id=context.child.id, level_id=level.id, is_current=True)
    context.db.add(cl); context.db.commit()
...
@given(u'孩子已获得{count:d}个成就')
def step_has_achievements(context, count):
    for i in range(count):
        ach = Achievement(name=f"成就{i+1}", type=Achievement.TYPE_BOOK_MILESTONE,
                          badge_emoji="📚")
        context.db.add(ach); context.db.commit()
        ca = ChildAchievement(child_id=context.child.id, achievement_id=ach.id)
        context.db.add(ca)
    context.db.commit()


@then(u'名片包含成就数量{count:d}')

...
def _ensure_book(context, title):
    ...
    if not hasattr(context, 'book_id') or not context.book_id:
        resp = context.client.get(f"/book/search?keyword={title}", headers=context.headers)
        data = resp.json()
...
def step_filter_ar(context, ar):
    context.response = context.client.get(f"/book/search?ar_level={ar}", headers=context.headers)
...
def step_filter_theme(context, theme):
    context.response = context.client.get(f"/book/search?theme={theme}", headers=context.headers)
...
def step_only_story_books(context):
    ...
    data = context.response.json()
    for book in data.get("items", data.get("list", [])):
        assert book.get("type", "story") == "story", f"Book {book.get('title')} is not story type"
...
def step_progress_created(context):
    resp = context.client.post("/reading/progress", params={
        "child_id": context.child.id, "book_id": context.book_id,
        "current_page": 1, "total_pages": 22,
...
@given(u'用户开始阅读"{title}"')
def step_start_reading_book(context, title):
    _ensure_book(context, title)
    context.client.post("/reading/session/start", params={
        "child_id": context.child.id, "book_id": context.book_id,
    }, headers=context.headers)
...
def step_read_10min(context):
    resp = context.client.post("/reading/session/start", json={
        "child_id": context.child.id, "book_id": context.book_id,
    }, headers=context.headers)
    if resp.status_code in (200, 201):
        sid = resp.json().get("id", resp.json().get("session_id"))
        context.client.put(f"/reading/session/{sid}/end", json={
            "pages_read": 5, "words_read": 800,
        }, headers=context.headers)
...
def step_reading_recorded(context):
    ...
    sessions = context.db.query(ReadingSession).filter(
...
def step_exit_app(context):
    ...
    context.client.post("/reading/progress", params={
        "child_id": context.child.id, "book_id": context.book_id,
        "current_page": 5, "total_pages": 22,
    }, headers=context.headers)
...
def step_reopen_app(context):
    ...
    context.response = context.client.get(f"/reading/progress/{context.book_id}",
                                          params={"child_id": context.child.id}, headers=context.headers)


@then('可以继续上次阅读位置')
def step_can_resume(context):
    resp = context.client.get(f"/reading/progress/{context.book_id}",
                              params={"child_id": context.child.id}, headers=context.headers)
    assert resp.status_code == 200
...
def step_click_word_in_text(context, word):
    ...
    existing = context.db.query(DictionaryWord).filter(DictionaryWord.word == word.lower()).first()
    if not existing:
        dw = DictionaryWord(word=word.lower(), chinese_meaning="华丽的",
                            phonetic="/ˈɡɔːrɡəs/", part_of_speech="形容词",
                            example_sentence="The sunset was gorgeous.")
        context.db.add(dw)
        context.db.commit()
    context.response = context.client.get(f"/vocabulary/lookup/{word}", headers=context.headers)

...
def step_bought_parent_course(context, price):
    ...
    context.db.commit()
    context.child = child
    order = Order(order_no="MW_REF_PC", user_id=context.user.id, child_id=child.id,
                  type=Order.TYPE_PARENT_COURSE, amount=price, pay_status=Order.PAY_PAID)
...
def step_attended_course(context):
    ...
    context.child = child
    order = Order(order_no="MW_REF_ATTENDED", user_id=context.user.id, child_id=child.id,
                  type=Order.TYPE_PARENT_COURSE, amount=99, pay_status=Order.PAY_PAID)
    context.db.add(order); context.db.commit()
    context.order = order
...
@given(u'用户为孩子购买观察期（实付{amount:d}元）')
def step_bought_observation(context, amount):
    child = Child(user_id=context.user.id, name="小明", age=7, grade="二年级")
    context.db.add(child)
    context.db.commit()
    context.child = child
    order = Order(order_no="MW_REF_OBS", user_id=context.user.id, child_id=child.id,
                  type=Order.TYPE_OBSERVATION, amount=amount, pay_status=Order.PAY_PAID)
    context.db.add(order)
    context.db.commit()
    context.order = order


@given(u'观察期已使用{days:d}天')
def step_obs_used_days(context, days):
    context.used_days = days


@then(u'退款金额为{amount:g}元（{formula}）')
def step_refund_amount_formula(context, amount, formula):
    ...
    if damage:
        # 有损坏罚款：验证退款申请中的金额
        resp = context.client.get(f"/refund/{context.refund_id}", headers=context.headers)
...
@given(u'用户为第二个孩子购买观察期（实付{amount:d}元，9折）')
def step_bought_obs_second_child(context, amount):
    step_bought_observation(context, amount)


@given(u'用户为孩子购买正式会员（实付{amount:d}元）')
def step_bought_official(context, amount):
    child = Child(user_id=context.user.id, name="小明", age=7, grade="二年级")
    context.db.add(child)
    context.db.commit()
    context.child = child
    order = Order(order_no="MW_REF_OFF", user_id=context.user.id, child_id=child.id,
                  type=Order.TYPE_OFFICIAL_MEMBER, amount=amount, pay_status=Order.PAY_PAID)
    context.db.add(order)
    context.db.commit()
    context.order = order


@given(u'正式会员已使用{days:d}天')
def step_official_used_days(context, days):
    context.used_days = days


@given(u'用户为第二个孩子购买正式会员（实付{amount:d}元，9折）')
...
@given(u'孩子有1本图书损坏（赔偿{amount:d}元）')
...
def step_refund_submitted(context):
    ...
    if not hasattr(context, 'order') or not context.order:
        ...
        context.db.add(child)
        context.db.commit()
        order = Order(order_no="MW_REF_AUDIT", user_id=context.user.id, child_id=child.id,
                      type=Order.TYPE_PARENT_COURSE, amount=99, pay_status=Order.PAY_PAID)
        context.db.add(order)
        context.db.commit()
        context.order = order

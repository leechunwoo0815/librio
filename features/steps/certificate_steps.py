...
def step_has_b_cert(context):
    _ensure_levels(context)
    svc = CertificateService(context.db)
    context.existing_cert = svc.generate_certificate(
        context.child.id, context.level_b.id
    )
...
def step_no_certs(context):
    # 确保孩子没有证书记录
    existing = (
        context.db.query(LevelCertificate)
        .filter(LevelCertificate.child_id == context.child.id)
        .count()
    )
    assert existing == 0

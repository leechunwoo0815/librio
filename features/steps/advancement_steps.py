...
def _ensure_level(context):
    """确保孩子有级别"""
    cl = (
        context.db.query(ChildLevel)
        .filter(ChildLevel.child_id == context.child.id, ChildLevel.is_current)
        .first()
    )
    if not cl:
        first = (
            context.db.query(Level)
        ...
        if first:
            cl = ChildLevel(
                child_id=context.child.id, level_id=first.id, is_current=True
            )
            context.db.add(cl)
            context.db.commit()
    return cl
...
def step_flip_to_last(context):
    # 直接 ORM 创建提交
    existing = (
        context.db.query(ReadingSubmission)
        .filter(
...
def step_submission_pending(context):
    sub = (
        context.db.query(ReadingSubmission)
        .filter(ReadingSubmission.id == context.submission_result["id"])
        .first()
    )
...
def step_teacher_notified(context):
    pending = (
        context.db.query(ReadingSubmission)
        .filter(ReadingSubmission.status == 0, ReadingSubmission.is_deleted == 0)
        .all()
    )
...
def step_admin_approve_submission(context):
    sub = (
        context.db.query(ReadingSubmission)
        .filter(ReadingSubmission.id == context.submission_id)
        .first()
    )
    ...
    cl = (
        context.db.query(ChildLevel)
        .filter(ChildLevel.child_id == sub.child_id, ChildLevel.is_current)
        .first()
    )
    if cl:
...
@given('图书"Charlotte\'s Web"已有3道选择题')
def step_book_has_questions(context):
    ...
    context.db.commit()
    context.questions = (
        context.db.query(QuestionBank)
        .filter(QuestionBank.book_id == context.book.id)
        .all()
    )
...
def step_teacher_view_questions(context):
    context.book_questions = (
        context.db.query(QuestionBank)
        .filter(QuestionBank.book_id == context.book.id, QuestionBank.is_deleted == 0)
        .all()
    )
...
def step_teacher_creates_quiz(context):
    ...
    context.quiz = svc.start_quiz(
        context.child.id, QuizStartRequest(book_id=context.book.id)
    )
...
def step_submit_all_correct(context):
    ...
    svc = AdvancementService(context.db)
    answers = [
        {"question_id": q.id, "answer": q.correct_answer} for q in context.questions
    ]
...
def step_submit_below_70(context):
    svc = AdvancementService(context.db)
    context.quiz_result = svc.submit_answers(
        context.quiz.id,
        [
...
def step_ranking_descending(context):
    if len(context.leaderboard) >= 2:
        assert (
            context.leaderboard[0]["books_read"] >= context.leaderboard[1]["books_read"]
        )


@then('孩子获得"{level_name}"徽章')

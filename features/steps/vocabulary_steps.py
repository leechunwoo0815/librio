...
def step_click_unknown_word(context, word):
    ...
    if not existing:
        dw = DictionaryWord(word=word.lower(), chinese_meaning="好奇心",
                            phonetic="/test/", part_of_speech="名词", example_sentence="Test.")
        context.db.add(dw)
        context.db.commit()
...
def step_add_to_vocab(context):
    ...
    if not existing:
        ...
        context.db.add(dw)
        context.db.commit()
    context.response = context.client.post("/vocabulary/", json={
        "child_id": context.child.id, "word": word,
    }, headers=context.headers)

@then(u'单词"{word}"加入生词本')
...
def step_enter_vocab_page(context):
    context.response = context.client.get(f"/vocabulary/{context.child.id}", headers=context.headers)
...
@when(u'用户将生词"{word}"标记为已掌握')
def step_mark_mastered(context, word):
    # Find the vocab entry and mark it
    resp = context.client.get(f"/vocabulary/{context.child.id}", headers=context.headers)
    words = resp.json()
    for w in words:
        if w["word"] == word.lower():
            context.response = context.client.put(f"/vocabulary/{w['id']}/master", headers=context.headers)
            return
...
def step_view_vocab_stats(context):
    context.response = context.client.get(f"/vocabulary/{context.child.id}/stats", headers=context.headers)

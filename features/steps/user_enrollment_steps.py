...
def step_order_status_paid(context):
    ...
    callback_resp = context.client.post(
        ...
        json={"order_no": data["order_no"], "trade_no": "WX_TEST_001", "pay_type": 1, "amount": data["amount"]},
        headers=context.headers,
    )
...
def step_child_done_parent_course(context):
    # 创建孩子并设置为已完成亲子课
    child = Child(
        user_id=context.user.id, name="小明", age=7, grade="二年级",
    ...
    # 创建已支付的亲子课程订单
    order = Order(
        order_no="MW_PC_DONE", user_id=context.user.id, child_id=child.id,
        type=Order.TYPE_PARENT_COURSE, amount=99.00, pay_status=Order.PAY_PAID,
    )
...
def step_click_pay_500(context):
    ...
    if context.response.status_code == 201:
        ...
        context.client.post(
            "/order/payment-callback",
            json={"order_no": data["order_no"], "trade_no": "WX_OBS", "pay_type": 1, "amount": data["amount"]},
            headers=context.headers,
        )
...
def step_child_not_done_parent_course(context):
    child = Child(
        user_id=context.user.id, name="小红", age=5, grade="幼儿园大班",
        status=Child.STATUS_TRIAL,
    )
    context.db.add(child)
    context.db.commit()
...
def step_try_access_observation(context):
    context.response = context.client.get(
        f"/child/{context.child.id}", headers=context.headers,
    )
    # 前端根据child.status判断是否可报观察期

# ==================== 正式会员报名 ====================

@given(u'用户的孩子观察期评估结果为"通过"')
def step_observation_passed(context):
    ...
    # 创建已支付的观察期订单
    order = Order(
        order_no="MW_OBS_DONE", user_id=context.user.id, child_id=child.id,
        type=Order.TYPE_OBSERVATION, amount=500.00, pay_status=Order.PAY_PAID,
    )
    context.db.add(order)
    context.db.commit()
...
def step_click_pay_5400(context):
    ...
    if context.response.status_code == 201:
        ...
        context.client.post(
            "/order/payment-callback",
            json={"order_no": data["order_no"], "trade_no": "WX_OFFICIAL", "pay_type": 1, "amount": data["amount"]},
            headers=context.headers,
        )
...
def step_already_has_official_member(context):
    ...
    existing_child = Child(
        user_id=context.user.id, name="大明", age=9, grade="四年级",
    ...
    # 已支付的观察期订单
    order = Order(
        order_no="MW_OBS_MULTI", user_id=context.user.id, child_id=child.id,
        type=Order.TYPE_OBSERVATION, amount=500.00, pay_status=Order.PAY_PAID,
    )
    context.db.add(order)
    context.db.commit()
...
def step_unlock_borrow(context):
    resp = context.client.get(
        f"/child/{context.child.id}/can-borrow", headers=context.headers,
    )

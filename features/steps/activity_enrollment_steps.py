...
@given(u'活动"{title}"为免费活动')
def step_free_activity(context, title):
    now = datetime.now()
    activity = Activity(
        title=title, type=Activity.TYPE_READING, max_participants=30,
        start_time=now + timedelta(days=7), end_time=now + timedelta(days=7, hours=2),
        venue_id=1, status=Activity.STATUS_ENROLLING,
    )
...
@given(u'活动"{title}"为收费活动（{price:d}元）')
def step_paid_activity(context, title, price):
    now = datetime.now()
    activity = Activity(
        title=title, type=Activity.TYPE_LECTURE, price=price, max_participants=50,
...
@given(u'活动"{title}"已报满')
def step_activity_full(context, title):
    now = datetime.now()
    activity = Activity(
        title=title, type=Activity.TYPE_READING, max_participants=1, current_participants=1,
        start_time=now + timedelta(days=7), end_time=now + timedelta(days=7, hours=2),
        venue_id=1,
    )
    context.db.add(activity)
...
@given(u'用户已报名免费活动"{title}"')
def step_enrolled_free_activity(context, title):
    now = datetime.now()
    activity = Activity(title=title, type=Activity.TYPE_READING, max_participants=30,
                        start_time=now + timedelta(days=7), end_time=now + timedelta(days=7, hours=2),
                        venue_id=1)
    context.db.add(activity)
    context.db.commit()
    ...
    context.db.commit()
    context.child = child
    enrollment = ActivityEnrollment(activity_id=activity.id, child_id=child.id, ticket_code="TKT123")
...
@given(u'用户已报名收费活动"{title}"（{price:d}元）')
...
def step_record_sign_in_time(context):
    ...
    enrollment = context.db.query(ActivityEnrollment).filter(
        ActivityEnrollment.id == context.enrollment.id
    ).first()
    assert enrollment is not None


@given(u'活动"{title}"因故取消')
...
@given(u'有{count:d}位用户已报名（其中{paid:d}位付费）')
...
@then(u'所有报名用户收到"活动取消"通知')
def step_all_notified(context):
    # 微信订阅消息推送，后端验证活动状态已更新
    from backend.domain.activity.models import Activity
    activity = context.db.query(Activity).filter(Activity.id == context.activity.id).first()

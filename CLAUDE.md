...
> **最后更新**：2026-08-09（52 题决策 + 四轮外部审查全闭环 + 终极全量审查 P0/P1/P2/P3 全部处置 + 160 轮审查 118 项修复五批次全闭环（audit-fix-batch-1~5）+ 终审整改（P1×3/P2×9/P3×14）闭环 + 决策表 V1.2（91 项）— 925 pytest collected（通过数随环境）/ 211/1369 behave + 56 表 + 67 配置 + 24 定时任务 + 336 API + CI 同构十一关全绿（含 Gate 11））
...
| **测试** | pytest (925 collected；通过数随环境：开发机 925 passed / CI 708+15 skip / 无 MySQL 沙箱 714+9 err) + behave (211 scenarios, 1369 steps, 0 failed) + Ruff (0 errors) |
| **API** | 336 个业务路由（include_router 注册，含 37 页面路由；不含 /health、/、/openapi.json，DEBUG mock 不计） |
| **领域模块** | 28 个 |
| **定时任务** | 24 个 |
...
| **RBAC 权限** | 三级：超管理员(136权限)/运营人员(108权限)/教师(29权限)。角色管理页面 `/admin/view/roles` 可视编辑权限。老师卡片底部可创建/编辑关联管理员账号。 |
...
| 11 | pytest | `venv/bin/python -m pytest tests/ -x -q --tb=short` | Exit Code 0 | 终端输出 |
| 12 | behave | `venv/bin/python -m behave features/ --no-capture -q` | Exit Code 0 | 终端输出 |
| 13 | ruff check | `venv/bin/ruff check backend/ tests/ && venv/bin/ruff check features/ scripts/` | Exit Code 0 | 终端输出 |
| 14 | ruff format | `venv/bin/ruff format --check .` | Exit Code 0 | 终端输出 |

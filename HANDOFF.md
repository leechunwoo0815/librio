# MegaWords (librio) 项目交接文档

> **生成时间**：2026-07-05 23:00 GMT+8
> **项目状态**：✅ 所有架构问题已修复，代码质量达标
> **下一步**：专家复查 → 生产部署
...
MegaWords (librio) 是一个儿童英语阅读管理平台：
...
**Service 层架构**：
```
backend/domain/admin/service.py → AdminService（管理端业务逻辑，含 17+ 管理方法）
backend/domain/advancement/service.py → AdvancementService（晋级域业务逻辑，含 7 查询方法）
...
| 文件 | 行数 | 职责 |
|------|------|------|
| `backend/domain/admin/routers/admin_venues_router.py` | ~60 | 场馆管理 |
| `backend/domain/admin/routers/admin_teachers_router.py` | ~140 | 老师管理 + 排班 |
| `backend/domain/admin/routers/admin_activities_router.py` | ~100 | 活动管理 |
| `backend/domain/admin/routers/admin_books_router.py` | ~200 | 图书 + 副本 + 上传 + 导出 |
| `backend/domain/admin/routers/admin_borrow_router.py` | ~160 | 借阅 + 押金 + 预约 |
| `backend/domain/admin/routers/admin_advancement_router.py` | ~210 | 级别 + 成就 + 证书 + 题库 + 审核 |
| `backend/domain/admin/routers/admin_reports_router.py` | ~140 | 退款 + 报告 + 阅读数据 |
| `backend/domain/admin/routers/admin_system_router.py` | ~320 | 仪表盘 + 配置 + 用户 + 订单 + 管理员 |

### 3.3 Service 文件

| 文件 | 新增方法 |
|------|----------|
| `backend/domain/admin/service.py` | list_deposits, list_reservations, list_refunds, approve_refund, list_observation_reports, get_reading_stats, get_reading_trends, get_order, mark_message_read, list_admins, get_admin, create_refund, create_order, update_order_status, delete_order, delete_message, get_refund_and_order |
| `backend/domain/advancement/service.py` | list_achievement_records, list_certificates, get_certificate, update_certificate, delete_certificate, list_submissions, list_questions |
...
|------|------|
| `backend/common/base_schema.py` | BaseSchema（extra="forbid"）+ PaginatedResponse + ApiResponse |
| `backend/domain/admin/admin_schemas.py` | 管理端 Schema（52 个，全部有 extra="forbid"） |
| `backend/domain/*/schemas.py` | 业务域 Schema（20 个，继承 BaseSchema） |
...
| 文件 | 修复内容 |
|------|----------|
| `backend/templates/admin/booklist.html` | 上传路径修复；JS 外迁到 `pages/booklist.js` |
| `backend/templates/admin/books.html` | 3 处上传路径修复；JS 外迁到 `pages/books.js` |
| `backend/templates/admin/reports.html` | JS 外迁到 `pages/reports.js` |
| `backend/templates/admin/orders.html` | JS 外迁到 `pages/orders.js` |
| `backend/templates/admin/borrow.html` | JS 外迁到 `pages/borrow.js` |
| `backend/templates/admin/activities.html` | JS 外迁到 `pages/activities.js` |
| `backend/templates/admin/levels.html` | JS 外迁到 `pages/levels.js` |
| `backend/static/admin/js/admin-pages.js` | 新增公共组件（确认弹窗/分页/表格状态/批量选择） |
| `backend/static/admin/js/pages/*.js` | 7 个页面独立 JS 模块 |

### 3.6 测试文件

| 文件 | 修复内容 |
|------|----------|
| `features/steps/deposit_steps.py` | 移除多余 amount 字段 |
| `features/steps/user_enrollment_steps.py` | 移除多余 amount 字段 |
...
4. **更新 `base.html`**
   - 全局引入 `admin-pages.js`（在 `admin.js` 之后）
...
### 高优先级
1. **专家复查** — 邀请专家对修复后的代码进行复查
2. **生产部署** — 部署到生产环境

### 中优先级
3. **API 限流** — 登录接口 5 次/分钟
4. **审计日志** — 关键操作自动记录
5. **数据库索引** — 高频查询字段
6. **剩余模板 JS 外迁** — 26 个非核心页面仍可继续拆分

### 低优先级
7. **前端缓存** — 图书列表等数据缓存
8. **API 版本管理** — `/api/v1/` 前缀

---

*交接文档更新时间：2026-07-05 23:00 GMT+8*

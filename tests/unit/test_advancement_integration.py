...
from backend.domain.advancement.models import Level, ChildLevel, QuestionBank, Quiz
...
def _setup(db):
    ...
    shelf = Bookshelf(
        child_id=child.id, book_id=book.id, status=Bookshelf.STATUS_BORROWING
    )
    db.add(shelf)
    db.commit()

    submission = ReadingSubmission(
        child_id=child.id, book_id=book.id, status=ReadingSubmission.STATUS_PENDING
    )
    db.add(submission)
    db.commit()
...
def test_quiz_pass_updates_word_count(db):
    """测验通过后更新单词积分"""
    user, child, book, level1, level2, cl, questions, shelf, submission, quiz = _setup(
        db
    )

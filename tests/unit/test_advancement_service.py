...
def test_check_and_advance_ready(db):
    ...
    cl = ChildLevel(
        ...
        level_id=level1.id,
        is_current=True,
        books_read_at_level=level1.required_books,
        quizzes_passed_at_level=5,
    )
    db.add(cl)
    db.commit()
...
def test_increment_books_read(db):
    ...
    _, child, _, level1, _, _ = _create_test_data(db)
    cl = ChildLevel(
        child_id=child.id, level_id=level1.id, is_current=True, books_read_at_level=0
    )
    db.add(cl)
    db.commit()
...
def test_grant_achievement(db):
    ...
    ach = Achievement(
        name="首次晋级",
        description="完成第一次晋级",
        type=Achievement.TYPE_LEVEL_UP,
        badge_emoji="⭐",
    )
    db.add(ach)
    db.commit()
...
def test_get_child_achievements(db):
    ...
    _, child, _, _, _, _ = _create_test_data(db)
    ach1 = Achievement(
        name="首次晋级", type=Achievement.TYPE_LEVEL_UP, badge_emoji="⭐"
    )
    ach2 = Achievement(
        name="读完10本", type=Achievement.TYPE_BOOK_MILESTONE, badge_emoji="📚"
    )

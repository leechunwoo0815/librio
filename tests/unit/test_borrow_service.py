...
def test_return_book(db):
    user, child, book = _setup(db)
    svc = BorrowService(db)
    borrow_result = svc.borrow_book(BorrowBookRequest(child_id=child.id, book_id=book.id))
    return_result = svc.return_book(ReturnBookRequest(borrow_record_id=borrow_result.id))

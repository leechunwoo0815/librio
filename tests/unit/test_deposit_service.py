...
from backend.domain.deposit.schemas import DepositPayRequest, DepositRefundRequest, DepositDeductRequest, DepositPayResponse
...
def _mock_gateway():
    gw = MagicMock()
    gw.create_order = AsyncMock(return_value=MagicMock(success=True, pay_params={"prepay_id": "mock_prepay_123"}))
...
async def test_pay_deposit(db):
    ...
    result = await svc.pay_deposit(DepositPayRequest(child_id=child.id), _mock_gateway(), current_user=user)
...
async def test_pay_deposit_duplicate(db):
    user, child = _setup(db)
    svc = DepositService(db)
    await svc.pay_deposit(DepositPayRequest(child_id=child.id), _mock_gateway(), current_user=user)

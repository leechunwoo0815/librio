...
def _setup(db):
    ...
    child = Child(user_id=user.id, name="押金测试", age=7, grade="二年级",
                  status=Child.STATUS_OFFICIAL, deposit_status=DepositStatus.UNPAID)
    db.add(child); db.commit()
    return user, child


def test_pay_deposit(db):
    user, child = _setup(db)
    svc = DepositService(db)
    result = svc.pay_deposit(DepositPayRequest(child_id=child.id))
    assert result.status == DepositStatus.PAID
    db.refresh(child)
    assert child.deposit_status == DepositStatus.PAID


def test_pay_deposit_duplicate(db):
    user, child = _setup(db)
    svc = DepositService(db)
    svc.pay_deposit(DepositPayRequest(child_id=child.id))
    with pytest.raises(Exception, match="已缴纳"):
        svc.pay_deposit(DepositPayRequest(child_id=child.id))


def test_refund_deposit_success(db):
    user, child = _setup(db)
    svc = DepositService(db)
    svc.pay_deposit(DepositPayRequest(child_id=child.id))
    result = svc.refund_deposit(DepositRefundRequest(child_id=child.id))
    assert result.status == DepositStatus.REFUNDING
    db.refresh(child)
    assert child.deposit_status == DepositStatus.REFUNDING


def test_refund_deposit_with_active_borrows(db):
    user, child = _setup(db)
    book = Book(isbn="978001", title="Test", author="A", ar_value=2.0, age_min=5, age_max=9, word_count=1000)
    db.add(book); db.commit()
    svc = DepositService(db)
    svc.pay_deposit(DepositPayRequest(child_id=child.id))
    borrow = BorrowRecord(child_id=child.id, book_id=book.id,
                          status=BorrowStatus.BORROWING,
                          borrow_time=datetime.now(),
                          due_date=datetime.now())
    db.add(borrow); db.commit()
    with pytest.raises(Exception, match="归还"):
        svc.refund_deposit(DepositRefundRequest(child_id=child.id))


def test_refund_deposit_with_fines(db):
    user, child = _setup(db)
    svc = DepositService(db)
    svc.pay_deposit(DepositPayRequest(child_id=child.id))
...
def test_deduct_deposit(db):
    user, child = _setup(db)
    svc = DepositService(db)
    svc.pay_deposit(DepositPayRequest(child_id=child.id))
    result = svc.deduct_deposit(DepositDeductRequest(child_id=child.id, amount=Decimal("120"), reason="丢书"))
...
def test_deduct_deposit_exceeds_balance(db):
    user, child = _setup(db)
    svc = DepositService(db)
    svc.pay_deposit(DepositPayRequest(child_id=child.id))
    with pytest.raises(Exception, match="超过"):
        svc.deduct_deposit(DepositDeductRequest(child_id=child.id, amount=Decimal("2000"), reason="巨额"))

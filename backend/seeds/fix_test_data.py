...
def fix_data():
    ...
    all_children = db.query(Child).filter(Child.is_deleted == 0).order_by(Child.id).all()
    ...
    for phone, parent_name in sample_users:
        ...
        if not user:
            user = User(openid=f"test_openid_{phone[-4:]}", phone=phone, parent_name=parent_name)
            db.add(user)
            db.flush()
    ...
    remaining_free = [c for c in free_children if c.user_id == original_user.id and c.id not in used_child_ids]
    ...
    # 4. 软删除多余自由孩子
    to_delete = [c for c in free_children if c.user_id == original_user.id and c.id not in used_child_ids]
    ...
    for c in to_delete:
        if c.id in assigned_ids:
            continue
        if kept_for_original > 0 and c.id in {cc.id for cc in remaining_free[:kept_for_original]}:
            continue
        c.is_deleted = 1
        db.add(c)
        deleted_count += 1
    ...
    now = datetime.now()
    activity = db.query(Activity).filter(Activity.status == Activity.STATUS_ENROLLING, Activity.is_deleted == 0).first()
    if not activity:
        activity = Activity(
            title="周末绘本阅读沙龙",
    ...
    # 给活动添加 5 条已报名记录
    enrolled_child_ids = {e.child_id for e in db.query(ActivityEnrollment).filter(
        ActivityEnrollment.activity_id == activity.id, ActivityEnrollment.is_deleted == 0
    ).all()}
    available_children = db.query(Child).filter(
        Child.is_deleted == 0, ~Child.id.in_(list(enrolled_child_ids) or [0])
    ).limit(10).all()
    added = 0
    ...
    if active_count < 3:
        ...
        children_for_resv = db.query(Child).filter(
            Child.is_deleted == 0,
    ...
    children_for_teachers = db.query(Child).filter(
        Child.is_deleted == 0, Child.teacher_id.is_(None)
    ).limit(len(teachers) * 3).all()
    for i, teacher in enumerate(teachers):
        ...
        # 创建两条排班
        existing_schedules = db.query(TeacherSchedule).filter(
            TeacherSchedule.teacher_id == teacher.id
        ).count()
        if existing_schedules == 0:
            db.add(TeacherSchedule(
                teacher_id=teacher.id, weekday=(i % 5) + 1,
                start_time="09:00", end_time="12:00"
            ))
            db.add(TeacherSchedule(
                teacher_id=teacher.id, weekday=(i % 5) + 3,
                start_time="14:00", end_time="17:00"
            ))
    logger.info(f"  ✅ 老师: 为 {len(teachers)} 位老师分配孩子和排班")

    # 8. 给没有分配场馆的孩子分配场馆
    children_no_venue = db.query(Child).filter(
        Child.is_deleted == 0, Child.venue_id.is_(None)
    ).all()
    if children_no_venue:
        ...
        logger.info(f"  ✅ 场馆: 为 {len(children_no_venue)} 个孩子分配场馆 (共 {len(venues)} 个场馆)")
    else:
...
def random_child_name(seed_id: int) -> str:
    names = [
        "艾米", "鲍勃", "查理", "黛西", "艾拉", "菲利克斯", "格蕾丝", "亨利",
        "伊娃", "杰克", "凯莉", "利奥", "玛雅", "诺亚", "奥利维亚", "彼得",
        "昆西", "露西", "山姆", "蒂娜", "维克多", "温迪", "赞恩", "佐伊",
    ]

...
class RefundRepository(BaseRepository[RefundApplication]):
    ...
    def get_by_user(self, user_id: int, page: int = 1, page_size: int = 20) -> tuple[list[RefundApplication], int]:
        q = self.db.query(RefundApplication).filter(
            RefundApplication.user_id == user_id,
            RefundApplication.is_deleted == 0,
        )
        total = q.count()
        records = q.order_by(RefundApplication.create_time.desc()).offset((page - 1) * page_size).limit(page_size).all()

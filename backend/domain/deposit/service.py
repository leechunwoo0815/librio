...
class DepositService:
    ...
    async def pay_deposit(
        ...
        try:
            ...
            logger.error(
                f"pay_deposit gateway error: child={data.child_id}, error={e}",
                exc_info=True,
            )
    ...
    async def repay_deposit(
        ...
        try:
            ...
            logger.error(
                f"repay_deposit gateway error: child={child_id}, error={e}",
                exc_info=True,
            )
    ...
    async def audit_refund(
        ...
        if payment_gateway:
            ...
            try:
                ...
                logger.error(
                    f"Refund failed: child={child_id}, error={e}", exc_info=True
                )
    ...
    async def partial_refund_deposit(
        ...
        if payment_gateway:
            try:
                ...
                logger.error(
                    f"Partial refund failed: child={child_id}, error={e}", exc_info=True
                )
                record = (
                    self.db.query(DepositRecord)
                    .filter(DepositRecord.id == record.id)
    ...
    async def pay_fines(
        ...
        try:
            ...
            logger.error(
                f"pay_fines gateway error: child={data.child_id}, error={e}",
                exc_info=True,
            )
            raise PaymentError(f"支付网关调用失败: {e}")
        if not result.success:

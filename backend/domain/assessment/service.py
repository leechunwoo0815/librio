...
class AssessmentService:
    ...
    def list_assessments(
        ...
        total = query.count()
        items = query.order_by(Assessment.create_time.desc()).offset((page - 1) * page_size).limit(page_size).all()

        result = []
        for a in items:
            child = self.db.query(Child).filter(Child.id == a.child_id, Child.is_deleted == 0).first()
            teacher = self.db.query(Teacher).filter(Teacher.id == a.teacher_id, Teacher.is_deleted == 0).first() if a.teacher_id else None
            venue = self.db.query(Venue).filter(Venue.id == a.venue_id, Venue.is_deleted == 0).first() if a.venue_id else None
        ...
        # 本月评估数
        month_total = self.db.query(func.count(Assessment.id)).filter(
            Assessment.is_deleted == 0,
            Assessment.create_time >= month_start
        ).scalar() or 0

        # 待评估数
        pending = self.db.query(func.count(Assessment.id)).filter(
            Assessment.is_deleted == 0,
            Assessment.status == "pending"
        ).scalar() or 0

        # 平均 AR 变化
        avg_ar_change = self.db.query(
        ...
        # 平均理解正确率
        avg_accuracy = self.db.query(
            func.avg(Assessment.comprehension_score)
        ).filter(
            Assessment.is_deleted == 0,
            Assessment.comprehension_score.isnot(None)
        ).scalar()
    ...
    def get_assessment(self, assessment_id: int) -> AssessmentResponse:
        """获取评估详情"""
        a = self.db.query(Assessment).filter(Assessment.id == assessment_id, Assessment.is_deleted == 0).first()
        if not a:
    ...
    def update_assessment(self, assessment_id: int, data: AssessmentUpdateRequest) -> AssessmentResponse:
        """更新评估"""
        assessment = self.db.query(Assessment).filter(Assessment.id == assessment_id, Assessment.is_deleted == 0).first()
        if not assessment:
            raise NotFoundError("评估记录不存在")

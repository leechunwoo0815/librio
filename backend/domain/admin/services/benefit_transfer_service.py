...
class BenefitTransferAdminService:
    ...
    def get_list(self, status: int | None = None, page: int = 1, page_size: int = 20) -> dict:
        ...
        items = query.order_by(
            BenefitTransferApplication.create_time.desc()
    ...
    def approve(self, application_id: int, reviewer_id: int, review_remark: str = "") -> dict:
        app = self.db.query(BenefitTransferApplication).filter(
        ...
        self.db.commit()
        logger.info(f"Benefit transfer approved: application_id={application_id}, reviewer_id={reviewer_id}")
        return {"success": True, "message": "审核通过，权益已转移"}

    def reject(self, application_id: int, reviewer_id: int, review_remark: str = "") -> dict:
        app = self.db.query(BenefitTransferApplication).filter(
            BenefitTransferApplication.id == application_id,
            BenefitTransferApplication.is_deleted == 0,
        ).first()
        ...
        app.reviewed_at = func.now()
        self.db.commit()
        logger.info(f"Benefit transfer rejected: application_id={application_id}, reviewer_id={reviewer_id}")

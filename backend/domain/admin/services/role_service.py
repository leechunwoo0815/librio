...
class AdminRoleService:
    ...
    def list_roles(self) -> dict:
        roles = self.db.query(Role).filter(Role.is_deleted == 0).order_by(Role.sort_order).all()
        from sqlalchemy import func
        result = []
        for r in roles:
            count = self.db.query(func.count(RolePermission.id)).filter(
    ...
    def get_role(self, role_id: int) -> dict:
        role = self.db.query(Role).filter(Role.id == role_id, Role.is_deleted == 0).first()
        if not role:
            raise NotFoundError("角色不存在")

        perm_codes = {
            rp.permission_code
            for rp in self.db.query(RolePermission).filter(
    ...
    def get_all_permissions(self, role_id: int | None = None) -> dict:
        perms = self.db.query(Permission).filter(Permission.is_deleted == 0).order_by(
            Permission.group_name, Permission.sort_order
        ).all()
        ...
        return {
            "groups": [{"group_name": g, "permissions": items} for g, items in groups.items()],
            "total": len(perms),
        }
    ...
    def set_role_permissions(self, role_id: int, permission_codes: list[str]) -> dict:
        ...
        for code in permission_codes:
            perm = self.db.query(Permission).filter(
                Permission.code == code, Permission.is_deleted == 0
            ).first()
            if perm:
    ...
    def create_role(self, code: str, name: str, description: str | None = None, sort_order: int = 0) -> dict:
        existing = self.db.query(Role).filter(Role.code == code, Role.is_deleted == 0).first()
        if existing:
        ...
        return {"id": role.id, "code": role.code, "name": role.name, "message": "角色创建成功"}

    def update_role(self, role_id: int, name: str | None = None, description: str | None = None, sort_order: int | None = None) -> dict:
        role = self.db.query(Role).filter(Role.id == role_id, Role.is_deleted == 0).first()
        if not role:
            raise NotFoundError("角色不存在")
    ...
    def delete_role(self, role_id: int) -> dict:
        ...
        admin_count = self.db.query(Admin).filter(
            Admin.admin_role_id == role_id, Admin.is_deleted == 0
        ).count()
        if admin_count > 0:

...
def _drop_indexes_safe():
    ...
    try:
        op.drop_index(
            op.f("ix_refund_application_user_id"), table_name="refund_application"
        )
    except Exception:
        pass
    try:
...
def _add_indexes_safe():
    try:
        op.create_index(
            "ix_refund_application_child_id", "refund_application", ["child_id"]
        )
    except Exception:
        pass
...
def _add_fk_safe():
    try:
        op.create_foreign_key(
            "fk_reservation_child", "reservation", "child", ["child_id"], ["id"]
        )
    except Exception:
        pass
    try:
        op.create_foreign_key(
            "fk_reservation_book", "reservation", "book", ["book_id"], ["id"]
        )
    except Exception:
        pass
...
def _make_not_null(table: str, column: str, col_type):
    try:
        op.execute(
            f"ALTER TABLE {table} MODIFY {column} {col_type.compile(dialect=mysql.dialect())} NOT NULL"
        )
    except Exception:
        pass

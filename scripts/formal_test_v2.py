...
def test(
    ...
    try:
        ...
        if ok:
            ...
            FAIL += 1
            ERRORS.append(
                f"[{role}] {name}: HTTP {status} (expected {expected_status}) - {json.dumps(body, ensure_ascii=False)[:150]}"
            )
        body_preview = (
        ...
        return (
            body if isinstance(body, dict) else (body if isinstance(body, list) else {})
        )
...
children = requests.get(
    f"{BASE}/child/", headers={"Authorization": f"Bearer {UT}"}
).json()
child_id = children[0]["id"] if isinstance(children, list) and children else None
print(f"  child_id={child_id}")

books = requests.get(
    f"{BASE}/book/search?page=1&page_size=1", headers={"Authorization": f"Bearer {UT}"}
).json()
book_id = (
    books.get("items", [{}])[0].get("id")
...
test(
    "用户列表",
    "GET",
    "/admin/api/users?page=1&page_size=10",
    token=AT,
    expected_status=200,
    role="管理员",
)
...
test(
    "订单列表(管理端)",
    "GET",
    "/admin/api/orders?page=1&page_size=10",
    token=AT,
    expected_status=200,
    role="管理员",
)
...
print("\n--- 1.7 场馆管理 ---")
test(
    "场馆列表", "GET", "/admin/api/venues", token=AT, expected_status=200, role="管理员"
)
...
print("\n--- 1.9 系统配置 ---")
test(
    "系统配置", "GET", "/admin/api/config", token=AT, expected_status=200, role="管理员"
)
...
if child_id:
    ...
    test(
        ...
        "/child/",
        token=UT,
        json_data={"name": "正式测试孩子", "age": 7, "grade": "一年级"},
        expected_status=201,
        role="家长",
    )
...
test(
    "按AR等级搜索",
    "GET",
    "/book/search?ar_level_min=3&ar_level_max=5&page=1",
    token=UT,
    expected_status=200,
    role="家长",
)
...
if child_id:
    test(
        "押金状态",
        "GET",
        f"/deposit/status?child_id={child_id}",
        token=UT,
        expected_status=200,
        role="家长",
    )
...
test(
    ...
    "/child/transfer",
    token=UT,
    json_data={"source_child_id": 1, "target_child_id": 2},
    role="家长",
)
...
if not child_id:
    ...
    if book_id:
        ...
        test(
            "获取阅读进度",
            "GET",
            f"/reading/progress/{child_id}/{book_id}",
            token=UT,
            role="孩子",
        )
        test(
    ...
    test(
        "打卡记录",
        "GET",
        f"/reading/checkin/{child_id}?year=2026&month=6",
        token=UT,
        expected_status=200,
        role="孩子",
    )
    ...
    test(
        ...
        "/vocabulary/",
        token=UT,
        json_data={"child_id": child_id, "word": "elephant", "book_id": book_id},
        expected_status=201,
        role="孩子",
    )
    ...
    test(
        "孩子成就",
        "GET",
        f"/advancement/achievements/{child_id}",
        token=UT,
        expected_status=200,
        role="孩子",
    )
    ...
    if book_id:
        test(
            "开始测验",
            "POST",
            f"/advancement/quiz/start?child_id={child_id}",
            token=UT,
            json_data={"book_id": book_id},
            expected_status=201,
            role="孩子",
        ...
        test(
            "获取题目",
            "GET",
            f"/advancement/quiz/questions/{book_id}",
            token=UT,
            expected_status=200,
            role="孩子",
        )
    ...
    test(
        "统计摘要",
        "GET",
        f"/report/stats/summary?child_id={child_id}",
        token=UT,
        role="孩子",
    )
    test(
        "今日统计",
        "GET",
        f"/report/stats/today?child_id={child_id}",
        token=UT,
        role="孩子",
    )
    test(
        "趋势数据",
        "GET",
        f"/report/stats/trend?child_id={child_id}&days=7",
        token=UT,
        role="孩子",
    )
    test(
        "周报告",
        "GET",
        f"/report/stats/weekly?child_id={child_id}",
        token=UT,
        role="孩子",
    )
...
test(
    "负数page",
    "GET",
    "/book/search?page=-1&page_size=10",
    token=UT,
    role="边界",
    note="应返回422而非500",
)
test(
    "超大page_size",
    "GET",
    "/book/search?page=1&page_size=9999",
    token=UT,
    role="边界",
    note="应返回422或限制大小",
)
...
api_alignment = [
    ...
    (
        "/advancement/quiz/questions/{bid}",
        "/advancement/quiz/questions/{bid}",
        "GET",
        "一致",
    ),
    (
        "/advancement/quiz/{qid}/submit",
        "/advancement/quiz/{qid}/submit",
        "POST",
        "一致",
    ),
...
for path, method, desc in missing_checks:
    # 用真实参数测试
    test_path = (
        path.replace("{cid}", str(child_id or 1))
        .replace("{bid}", str(book_id or 1))
        .replace("{w}", "cat")
        .replace("{rid}", "1")
        .replace("{id}", "1")
    )

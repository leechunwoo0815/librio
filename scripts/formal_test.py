...
def test(name, method, path, token=None, json_data=None, expected_status=None, role="unknown"):
    ...
    try:
        if method == "GET":
            ...
            r = requests.put(f"{BASE}{path}", headers=headers, json=json_data, timeout=10)
        elif method == "DELETE":
        ...
        if ok:
            ...
            FAIL += 1
            ERRORS.append(f"[{role}] {name}: HTTP {status} - {json.dumps(body, ensure_ascii=False)[:150]}")
        ...
        RESULTS.append({
            ...
            "status": status,
            "ok": ok,
            "body_preview": json.dumps(body, ensure_ascii=False)[:100] if isinstance(body, (dict, list)) else str(body)[:100]
        })
...
# 测试所有管理端HTML页面是否可访问
admin_pages = [
    "/admin/", "admin/dashboard", "admin/users", "admin/books",
    "admin/orders", "admin/borrow", "admin/deposit", "admin/activities",
    "admin/teachers", "admin/settings", "admin/levels", "admin/submissions",
    "admin/activity_checkin", "admin/achievements", "admin/bookcopy",
    "admin/booklist", "admin/questions", "admin/reports", "admin/reservation",
    "admin/venues"
]
